...
namespace PTaMLab2.Files
{
    public class VirtualMemory : IDisposable
    {
        ...
        /// <summary> Метод определения номера (индекса) слота буфера, где находится элемент 
        /// с заданным индексом. Реализует алгоритм замещения FIFO </summary>
        private int GetPageBufferIndex(long elementIndex)
        {
            long absPage = elementIndex / _elemsPerPage;

            // Проверяем наличие нужной страницы в буфере
            for (int i = 0; i < BUFFER_SIZE; i++)
            { 
                if (_buffer[i].AbsolutePageNumber == absPage) return i;
            }

            // Если отсутствует, выбираем самый старый слот для замещения
            int victim = 0;
            for (int i = 1; i < BUFFER_SIZE; i++) 
            { 
                if (_buffer[i].LoadTime < _buffer[victim].LoadTime) victim = i;
            }

            // Если страница модифицирована, выгружаем её в файл
            if (_buffer[victim].Modified == 1) FlushPage(victim);

            // Загружаем новую страницу, обновляем атрибуты
        ...
        private void ValidateSwapFile()
        {
            ...
            {
                throw new InvalidDataException("Неверная сигнатура файла (ожидалось 'VM').");
            }
            ...
            if (storedSize != _size) 
            {
                throw new InvalidDataException($"Размер массива в файле ({storedSize}) не совпадает с запрошенным ({_size}).");
            }

            char storedType = (char)_swapFile.ReadByte();
            char expectedType = _arrayType == "int" ? 'I' : _arrayType == "char" ? 'C' : 'V';
            if (storedType != expectedType) 
            { 
                throw new InvalidDataException($"Тип элемента в файле ('{storedType}') не совпадает с ожидаемым ('{expectedType}').");
            }
            ...
            { 
                throw new InvalidDataException($"Длина строки в файле ({storedStrLen}) не совпадает с запрошенной ({_strLen}).");
            }
        }
        ...
        /// <summary>Проверяет, что индекс находится в диапазоне.</summary>
        private void ValidateIndex(long index)
        {
            if (index < 0 || index >= _size)
            {   
                throw new IndexOutOfRangeException($"Индекс {index} выходит за границы массива [0, {_size - 1}].");
            }
        }

        /// <summary>Выравнивает значение value на ближайшее кратное 512 сверху.</summary>
        private static int AlignTo512(int value) => (value + BASE_PAGE_BYTES - 1) / BASE_PAGE_BYTES * BASE_PAGE_BYTES;
        ...
        public VirtualMemory(string filePath, long size, string arrayType = "int", int strLen = 0)
        {
            if (size <= 0)
            {
                throw new ArgumentOutOfRangeException(nameof(size), "Размерность массива должна быть > 0.");
            }
            ...
            switch (_arrayType)
            {
                ...
                case "char":
                    if (strLen <= 0)
                    {
                        throw new ArgumentException("Для типа char длина строки должна быть > 0.", nameof(strLen));
                    }
                    _elemsPerPage = STR_ELEMS_PER_PAGE; // 128
                    _bitmapBytes = STR_BITMAP_BYTES; //  16
                    // Размер данных: 128 × strLen, выравнять на 512
                ...
                case "varchar":
                    if (strLen <= 0)
                    {
                        throw new ArgumentException("Для типа varchar максимальная длина строки должна быть > 0.", nameof(strLen));
                    }
                    _elemsPerPage = STR_ELEMS_PER_PAGE; // 128
                    _bitmapBytes = STR_BITMAP_BYTES; // 16
                ...
                default:
                    throw new ArgumentException($"Неизвестный тип массива: '{arrayType}'. " + "Ожидается int, char или varchar.", nameof(arrayType));
            }

            // Количество страниц: выравнивание размера массива на границу страницы
            _pageCount = (_size + _elemsPerPage - 1) / _elemsPerPage;

            // Открываем файл подкачки в режиме rw
            bool isNewFile = !File.Exists(filePath) || new FileInfo(filePath).Length == 0;
            _swapFile = new FileStream(filePath, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite);

            if (isNewFile) InitSwapFile();    // создаём структуры, заполняем нулями
            else ValidateSwapFile(); // проверяем сигнатуру и параметры

            // Для varchar — открываем файл строк (.dat)
            if (_arrayType == "varchar")
            {
                string datPath = Path.ChangeExtension(filePath, ".dat");
                _datFile = new FileStream(datPath, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite);
            }
        ...
        /// <summary> Метод чтения значения элемента массива с заданным индексом. </summary>
        public long ReadInt(long index)
        {
            ...
            if (!_buffer[bufSlot].IsBitSet(slotOnPage)) return 0L;

            int dataOffset = slotOnPage * INT_ELEM_BYTES;
            return BitConverter.ToInt64(_buffer[bufSlot].Data, dataOffset);
        }

        /// <summary> Метод записи заданного значения в элемент массива с указанным индексом. </summary>
        public void WriteInt(long index, long value)
        {
            ...
            Array.Copy(BitConverter.GetBytes(value), 0, _buffer[bufSlot].Data, dataOffset, INT_ELEM_BYTES);
        ...
        {
            char typeChar = _arrayType == "int" ? 'I' : _arrayType == "char" ? 'C' : 'V';
            ...
            if (_strLen > 0) Console.WriteLine($"  Длина строки       : {_strLen} байт");
            ...
            if (_datFile != null) Console.WriteLine($"  Размер .dat файла  : {_datFile.Length} байт");
        }
        ...
        public void Dispose()
        {
            for (int i = 0; i < BUFFER_SIZE; i++)
            { 
                if (_buffer[i].Modified == 1) FlushPage(i);
            }
